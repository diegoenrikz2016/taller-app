...
import { sampleWithRequiredData } from '../orden-trabajo.test-samples';
...
describe('OrdenTrabajo Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: OrdenTrabajo;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 14189 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.ordenTrabajos()[0]).toEqual(expect.objectContaining({ id: 14189 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 14189 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.ordenTrabajos()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 14189 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to ordenTrabajoService', () => {
      ...
      vitest.spyOn(service, 'getOrdenTrabajoIdentifier');
      const id = comp.trackId(entity);
      expect(service.getOrdenTrabajoIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.ordenTrabajosParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
