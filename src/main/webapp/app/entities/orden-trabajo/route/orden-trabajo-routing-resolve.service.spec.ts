...
import ordenTrabajoResolve from './orden-trabajo-routing-resolve.service';

describe('OrdenTrabajo routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IOrdenTrabajo returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          ordenTrabajoResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(ordenTrabajoResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
