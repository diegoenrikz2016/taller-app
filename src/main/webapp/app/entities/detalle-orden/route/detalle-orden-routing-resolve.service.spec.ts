...
import detalleOrdenResolve from './detalle-orden-routing-resolve.service';
...
describe('DetalleOrden routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IDetalleOrden returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          detalleOrdenResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(detalleOrdenResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
