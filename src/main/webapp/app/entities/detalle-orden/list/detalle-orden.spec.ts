...
import { sampleWithRequiredData } from '../detalle-orden.test-samples';
...
describe('DetalleOrden Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 21649 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.detalleOrdens()[0]).toEqual(expect.objectContaining({ id: 21649 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 21649 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.detalleOrdens()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 21649 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to detalleOrdenService', () => {
      const entity = { id: 21649 };
      vitest.spyOn(service, 'getDetalleOrdenIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.detalleOrdensParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
